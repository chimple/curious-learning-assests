...
# --- New grouping by app type ---
APP_TYPE_MAP = {
    'cr_lang': 'Feed The Monster',
    'book': 'StoryBook',
    'data': 'Assessment',
}
APP_TYPE_FILENAME = {
    'cr_lang': 'feedthemonster',
    'book': 'storybook',
    'data': 'assessment',
}
...
def get_app_type(app):
    ...
    if 'data=' in app_url:
        return 'data'
    elif 'cr_lang=' in app_url:
        return 'cr_lang'
    elif 'book=' in app_url:
        return 'book'
    else:
        return None
...
def main():
    ...
    # Group by app type
    app_type_to_apps = {k: [] for k in APP_TYPE_MAP.keys()}
    for app in languages.get("web_apps", []):
        app_type = get_app_type(app)
        if app_type in app_type_to_apps:
            app_type_to_apps[app_type].append(app)

    # Generate navigation and sub-catalogs for each app type
    for app_type, apps in app_type_to_apps.items():
        if not apps:
            continue
        display_name = APP_TYPE_MAP[app_type]
        filename = APP_TYPE_FILENAME[app_type]
        nav_entry = {
            "href": f"grades/{filename}.json",
            "title": display_name,
            "type": "application/opds+json",
            "alternate": [
                {
                    "href": apps[0]["appIconUrl"],
                    "rel": "icon",
                    "type": "image/png",
                    "title": display_name
                }
        ...
        # Generate the sub-catalog for this app type with all its apps
        # Use app_type as lang_code for compatibility with generate_grade_catalog
        grade_catalog = generate_grade_catalog(apps, app_type)
        grade_path = os.path.join(GRADES_DIR, f"{filename}.json")
